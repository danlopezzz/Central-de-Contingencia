-- ============================================================
-- Quiz Trading Pro — VARIAÇÃO C — rastreamento
--
-- Rode no SQL Editor de um projeto Supabase NOVO, exclusivo do quiz C.
-- Não use o projeto do quiz A nem o do quiz B: cada variação tem o seu
-- próprio banco, para nunca haver chance de misturar os números.
--
-- Este é o PRIMEIRO dos três arquivos. A ordem é:
--   1. supabase-tracking-c.sql   (este)
--   2. supabase-popup-c.sql
--   3. supabase-ftd-c.sql        (opcional, só se for medir depósito)
-- ============================================================
...
-- 3. PERMISSÕES
--
--    O painel abre sem senha, então a leitura fica liberada para quem
--    tiver o link e a chave anon (que é pública por natureza, pois fica
--    no código do quiz).
--
--    quiz_config fica SEM policy nenhuma, de propósito: se o anônimo
--    pudesse escrever ali, qualquer pessoa que lesse o código do quiz
--    poderia apontar o webhook para o próprio servidor e passar a
--    receber os seus leads. O webhook se configura no Table Editor.
--
--    Os dados são anônimos: nenhum nome, telefone ou e-mail é coletado.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4. WEBHOOK (opcional)
--    Habilite pg_net em Database > Extensions antes de usar, e
--    descomente o create trigger no fim.
-- ------------------------------------------------------------
